...
{
    public class HellfireGreatbow : ModProjectile
    {
                ...
                {//run on first tick of releasing
                    ...
                    */
                    if (multSuperDamage != 1 && fullPower)
                    {
                        ...
                        for (int i = 0; i < 30; i++)
                        {
                            int dustIndex = Dust.NewDust(arrow.position, arrow.width, arrow.height, 6, arrow.velocity.X * (1.5f - i / 25f), arrow.velocity.Y * (1.5f - i / 25f), 100, default(Color), 0.8f);
