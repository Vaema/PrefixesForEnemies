...
{
    public class Greatbone : ModProjectile
    {
        ...
        float multKnockback = 0.45f; //x1.99
                ...
                {//run on first tick of releasing
                    ...
                    */
                    if (multSuperDamage != 1 && fullPower)
                    {
                        ...
                        {
                            arrow.maxPenetrate+=4;
                            arrow.penetrate+=4;
                        }
                        for (int i = 0; i < 40; i++)
                        {
                            int dustIndex = Dust.NewDust(arrow.position, arrow.width, arrow.height, 1, arrow.velocity.X * (1.5f - i / 25f), arrow.velocity.Y * (1.5f - i / 25f), 100, default(Color), 0.8f);
                        }
                    }
                    released = true;
                }
                    ...
                    if (fullPower && arrow.active)
                    {
                        ...
                        if (multSuperDamage != 1 && fullPower)
                        {
                            ...
                            dustIndex = Dust.NewDust(spawnPosition, 0, 0, 1, -10 * (float)Math.Cos(rotToTarget), -10 * (float)Math.Sin(rotToTarget), 0, Color.Transparent, 0.9f);
