...
{
    public class ShadowstrungGreatbow : ModProjectile
    {
                ...
                {//run on first tick of releasing
                    ...
                    */
                    if (multSuperDamage != 1 && fullPower)
                    {
                        ...
                        info.shadowstrung = true;
                        for (int i = 0; i < 30; i++)
                        {
                            int dustIndex = Dust.NewDust(arrow.position, arrow.width, arrow.height, 109, arrow.velocity.X * (1.5f - i / 25f), arrow.velocity.Y * (1.5f - i / 25f), 100, default(Color), 0.8f);
                    ...
                    if (fullPower && arrow.active)
                    {
                        ...
                        if (multSuperDamage != 1 && fullPower)
                        {
                            ...
                            dustIndex = Dust.NewDust(spawnPosition, 0, 0, 6, -10 * (float)Math.Cos(rotToTarget), -10 * (float)Math.Sin(rotToTarget), 0, Color.Transparent, 0.9f);
