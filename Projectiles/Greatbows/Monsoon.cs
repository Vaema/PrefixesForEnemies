...
{
    public class Monsoon : ModProjectile
    {
        ...
        float multKnockback = 0.35f; //x2.2
        ...
        public override void AI()
        {
            ...
            Projectile.position = player.Center - new Vector2(Projectile.width / 2, Projectile.height / 2 + (int)Projectile.ai[1]/2);
                ...
                {//run on first tick of releasing
                    ...
                    */
                    if (multSuperDamage != 1 && fullPower)
                    {
                        ...
                        float baseSpeed = arrow.velocity.Length();
                        double startAngle = Math.Atan2(arrow.velocity.X, arrow.velocity.Y) - spread / 2f;
                        ...
                        for (i = 0; i < 5; i++)
                        {
                            ...
                            int p = Projectile.NewProjectile(arrow.position.X, arrow.position.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), arrow.type, (int)(arrow.damage), arrow.knockBack, arrow.owner);
                            Main.projectile[p].localAI[0] = 1;
                            Main.projectile[p].noDropItem = true;
                        }
                        for (i = 0; i < 30; i++)
                        {
                            int dustIndex = Dust.NewDust(arrow.position, arrow.width, arrow.height, 156, arrow.velocity.X * (1.5f - i / 25f), arrow.velocity.Y * (1.5f - i / 25f), 100, default(Color), 0.8f);
                    ...
                    if (fullPower && arrow.active)
                    {
                        ...
                        if (multSuperDamage != 1 && fullPower)
                        {
                            ...
                            dustIndex = Dust.NewDust(spawnPosition, 0, 0, 156, -10 * (float)Math.Cos(rotToTarget), -10 * (float)Math.Sin(rotToTarget), 0, Color.Transparent, 0.9f);
