...
{
    public class LightcrystalGreatbow : ModProjectile
    {
        ...
        {
            Lighting.AddLight(Projectile.Center, .05f, .2f, .5f);
                ...
                {//run on first tick of releasing
                    ...
                    */
                    if (multSuperDamage != 1 && fullPower)
                    {
                        ...
                        info.lightcrystal = true;
                        for (int i = 0; i < 30; i++)
                        {
                            int dustIndex = Dust.NewDust(arrow.position, arrow.width, arrow.height, 69, arrow.velocity.X * (1.5f - i / 25f), arrow.velocity.Y * (1.5f - i / 25f), 100, default(Color), 0.8f);
                    ...
                    if (fullPower && arrow.active)
                    {
                        ...
                        if (multSuperDamage != 1 && fullPower)
                        {
                            ...
                            dustIndex = Dust.NewDust(spawnPosition, 0, 0, DustID.PinkCrystalShard, -10 * (float)Math.Cos(rotToTarget), -10 * (float)Math.Sin(rotToTarget), 0, Color.Transparent, 0.9f);
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            spriteBatch.Draw(Mod.GetTexture("Projectiles/Greatbows/Glow1"), Projectile.Center - Main.screenPosition, new Rectangle(0, Mod.GetTexture("Projectiles/Greatbows/Glow1").Height/5 * Projectile.frame, Projectile.width, Mod.GetTexture("Projectiles/Greatbows/Glow1").Height / 5), new Color(255, 255, 255), Projectile.rotation, new Vector2(Projectile.width/2, Mod.GetTexture("Projectiles/Greatbows/Glow1").Height/10), Projectile.scale, effect, 0f);
        }
    }
}