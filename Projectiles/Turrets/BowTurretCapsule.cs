...
{
    public class BowTurretCapsule : ModProjectile
    {
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            player.AddBuff(Mod.Find<ModBuff>("BowTurret").Type, 18002);
            MPlayer mplayer = (MPlayer)player.GetModPlayer(Mod, "MPlayer");
            mplayer.bowTurret = true;
            int p = Projectile.NewProjectile(Projectile.position.X, Projectile.position.Y, 0, 0, Mod.Find<ModProjectile>("BowTurret").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0, aiOut);
            return true;
        }
        public override void OnKill(int timeLeft)
        {
            if (timeLeft == 0 && Main.netMode != 1)
            {
