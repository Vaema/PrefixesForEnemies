...
{
    public class ShattershardSpawner : ModProjectile
    {
        ...
        {
            // DisplayName.SetDefault("ShattershardSpawner");
        }
        ...
        {
            Projectile.ai[0] += 1f;
            if (Projectile.ai[0] >= 10f && Projectile.owner == Main.myPlayer)
            {
                ...
                int p = Projectile.NewProjectile(posX, posY, XVel, YVel, Mod.Find<ModProjectile>("Shattershard").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
            }
        }
    }
}
