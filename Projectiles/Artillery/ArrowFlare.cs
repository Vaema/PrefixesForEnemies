        ...
        public override void AI()
        {
            if (Projectile.velocity.Length() == 0 && Main.player[Projectile.owner].FindBuffIndex(Mod.Find<ModBuff>("ArtilleryCooldown").Type)==-1 && Projectile.position.Y < Main.worldSurface)
            {
                for(int i=0; i<15; i++)
                {
                    ...
                    int p = Projectile.NewProjectile(X, Y, -X / 200, 5 + Main.rand.Next(-100, 101) / 100, 1, (int)(15 * Main.player[Projectile.owner].GetDamage(DamageClass.Ranged)), 2, Projectile.owner);
                }
