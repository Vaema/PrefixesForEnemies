...
{
    public abstract class HoverLOS : Minion
    {
        protected float idleAccel = 0.08f;
        protected float spacingMult = 1.1f;
        protected float viewDist = 500f;
        protected float chaseDist = 200f;
        protected float chaseAccel = 6f;
        protected float inertia = 40f;
        protected float shootCool = 140f;
        protected float shootSpeed;
        protected float floatDistance = 48;
        protected int shoot;
        ...
        public override void Behavior()
        {
            ...
            for (int k = 0; k < 1000; k++)
            {
                ...
                if (k != Projectile.whoAmI && otherProj.active && otherProj.owner == Projectile.owner && otherProj.Name.Contains("Elemental Spirit") && System.Math.Abs(Projectile.position.X - otherProj.position.X) + System.Math.Abs(Projectile.position.Y - otherProj.position.Y) < spacing)
                {
