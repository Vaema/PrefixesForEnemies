    ...
    public override void SetStaticDefaults()
    {
      ...
      // Tooltip.SetDefault("Right-click to shoot.");
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.altFunctionUse == 2)
            {
                for (int i = 0; i < 4 + shotBonus / 3; i++)
                {
                    ...
                    Projectile.NewProjectile(position.X, position.Y, vX, vY, type, (int)(damage * .8 + (15 * shotBonus * (player.GetDamage(DamageClass.Melee) - 1))), knockBack, Main.myPlayer);
                }
                shotBonus = 0;
            }
            return false;
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(Mod.Find<ModItem>("GunbladeSpreadMkII").Type, 1);
